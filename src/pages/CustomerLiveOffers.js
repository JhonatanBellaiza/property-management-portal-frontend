...
function CustomerLiveOffers() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8080/api/customer/${userId}/liveOffers`)
  ...
  const handleOfferStatusChange = (offerId, newStatus) => {
    axios
      .get(
        `http://localhost:8080/api/customer/${userId}/offer/${offerId}?changeOfferStatus=${newStatus}`
      )
      .then((response) => {
...
export default CustomerLiveOffers
